...
func (w *Worker) RunOnce(ctx context.Context) (int, error) {
	...
	for i, job := range jobs {
		if ctx.Err() != nil {
			// Cancelled mid-batch: release the jobs we won't touch so they are
			// retried elsewhere instead of waiting for the lease to expire.
			w.nackRest(jobs[i:])
			break
		}
	...
	if len(results) > 0 {
		if _, err := w.Coord.Report(ctx, w.ID, results); err != nil {
			// Report failed: release the jobs so another worker retries them.
			w.nackRest(jobs[:len(results)])
			return 0, err
		}
	}
